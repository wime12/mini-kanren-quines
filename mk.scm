...
(define ext-A 
  (lambda (x tag pred S A)
    (cond
      ((null? A) (list (cons x (cons tag pred))))
      (else
       (let ((a (car A)) (A (cdr A)))
         (let ((a-tag (pr->tag a)))
           (cond
...
(define ext-T+ 
  (lambda (x tag pred S T)
    (cond
      ((null? T) (list (cons x (cons tag pred))))
      (else
       (let ((t (car T)))
         (let ((t-tag (pr->tag t)))
           (cond
    ...
    (let ((fd (drop-dot-D (sorter (map sorter D))))
      ...
      (let ((fb (append ft fa)))
        (cond
          ...
          ((null? fd) (cons v fb))
          ((null? fb) (cons v (list (cons '=/= fd))))
          (else (cons v (cons (cons '=/= fd) fb))))))))
